...
if [ -e ${FNAME} ]; then
 ...
 while read LINE; do
  H=`echo ${LINE}|awk -F',' '{print $1}'`
  if [ "$HIDUKE" != "$H" ]; then
   echo $LINE|awk -F',' '{printf("delete from ultra_clsdaysale where saleday=\x27%s\x27 and strcode=%s;",$1,$2);print ""}'>>sql.txt
  fi
  HIDUKE=${H}
 
  echo $LINE|awk -F',' '{printf("insert into ultra_clsdaysale(saleday,strcode,clscode,saleitem,saleamt,disitem,disamt,dispitem,dispamt) values(\x27%s\x27,%s,%s,%s,%s,%s,%s,%s,%s);",$1,$2,$3,$4,$5,$6,$7,$8,$9);print "";}'>>sql.txt
 done < ${FNAME}
...
if [ -e ${FNAME} ]; then
 ...
 while read LINE; do
  H=`echo ${LINE}|awk -F',' '{print $1}'`
  if [ "$HIDUKE" != "$H" ]; then
   echo $LINE|awk -F',' '{printf("delete from ultra_lindaysale where saleday=\x27%s\x27 and strcode=%s;",$1,$2);print ""}'>>sql.txt
  fi
  HIDUKE=${H}
 
  echo $LINE|awk -F',' '{printf("insert into ultra_lindaysale(saleday,strcode,lincode,saleitem,saleamt,disitem,disamt,dispitem,dispamt) values(\x27%s\x27,%s,%s,%s,%s,%s,%s,%s,%s);",$1,$2,$3,$4,$5,$6,$7,$8,$9);print "";}'>>sql.txt
 done < ${FNAME}
